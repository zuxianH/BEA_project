...
(* ::Item::Closed:: *)
(*GenerateQsystem[\[Lambda]_]:  Given yd generate QQ relation "Allrel" and its variables "vars". (standard input do not need to change)*)
...
(* ::Item::Closed:: *)
(*GenerateWsystem[\[Lambda]_]:  create vars from wronskian*)
...
(* ::Item::Closed:: *)
(*init\[Theta]rep[\[Lambda]T_]:  Function to initialize the \[Theta] representation for a given Young tableau \[Lambda]T. This will use in function Large\[CapitalLambda]ExpansionUptdStart[\[Lambda]T_] *)
...
(*FindSolUptdAsymptW := Block[{},
  {
    ...
    vars,
    Method -> "NelderMead",
    WorkingPrecision -> prec/2,
    PrecisionGoal -> prec/2,
    AccuracyGoal -> prec/4,
    MaxIterations -> \[Infinity]
  ];
];*)
...
IterateUptdInterimJulia[SYT_]:=Block[{},
 ...
 juliaOutput = ExternalEvaluate[sessionJulia,juliaInputNew[SYT,\[CapitalLambda]0Interim]];
...
RandomPartitionWithLength[20,2]
...
(* ::Input:: *)
(*savedJuliaTime = {};*)
(*singleSYTresultUNIK[{{1,2,5,7,8,9,10,11,14,15,18,19,20},{3,4,6,12,13},{16,17}}]*)
...
yd = RandomPartitionWithLength[20,5]
...
(* ::Input:: *)
(*savedJuliaTime = {};*)
(*singel1= singleSYTresult[yd,"rand"]//AbsoluteTiming*)
(*myrandTableau*)
...
function run_lazy(dp_tol::Float64,myOpts)
    run = run_continuation_with_tol(
        prob,
        PALC(tangent = Bordered(),\[Theta]=0.5), 
...
function cb(state; dp, kwargs...)
    ...
    if !fromNewton && _x !== nothing
        ...
        tol_vals = [0.05 * max(1, abs(x)) for x in state.x]
...
final_result = run_lazy(10. ,main_Opts)
...
(* ::Input:: *)
(*juliaOutput = ExternalEvaluate[sessionJulia,juliaInputNew[test,200]]*)
