...
Get[FileNameJoin[{$SUSYWBEProjectRoot, "wolfram", "SUSYWBEWorkflow.wl"}]];
...
BertiniContinuationScript[] := FileNameJoin[{
  ...
  "scripts", "continue_lambda0_to_zero.py"
}];
...
RunBertiniContinuation[SYT_] := Module[
  {
    ...
    timeout, command, processStart, processSeconds,
  ...
  (* RunProcess has exited and Python publishes the CSV atomically. *)
...
IterateUptdInterimBertini[SYT_] := Block[{},
  runConfiguration = GetJuliaRunConfiguration[];
  exportSeconds = First@AbsoluteTiming[exportToJuliaInitialData[SYT, False]];
  ...
  If[Length[bertiniOutput] =!= Length[susyvars],
    Throw[Nothing, "BadBertiniOutput"]
  ];
  minsolrep = Rule @@@ Transpose[{susyvars, bertiniOutput}];
  sol[0] = minsolrep;
...
subSYTResultDirectory = BertiniEnvString[
  ...
  FileNameJoin[{SUSYWBEProjectDirectory[], "workspace", "results"}]
];
