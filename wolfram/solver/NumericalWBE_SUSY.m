...
Get[FileNameJoin[{DirectoryName[DirectoryName[$InputFileName]], "Paths.wl"}]];


ClearAll[SUSYWBEUtilsDirectory, SUSYWBESolverDirectory, SUSYWBEJuliaDirectory];
SUSYWBEUtilsDirectory[] := FileNameJoin[{SUSYWBEPackageDirectory[], "wolfram", "utils"}];
SUSYWBESolverDirectory[] := FileNameJoin[{SUSYWBEPackageDirectory[], "wolfram", "solver"}];
...
FindSolUptdAsymptW::failed = "The asymptotic minimizer did not return a numeric solution. Stopping this tableau.";

FindSolUptdAsymptW := Module[{initialResiduals, minimumResult},
  (* Small tableaux can have an exact asymptotic starting solution. Calling
     FindMinimum at that point can return unevaluated after FindMinimum::fmgz.
     Only exact zero residuals qualify; no tolerance or precision is relaxed. *)
  initialResiduals = normedrel /. subnc;
  If[ListQ[initialResiduals] && AllTrue[initialResiduals, # === 0 &] &&
      VectorQ[vars /. subnc, NumericQ],
    minim = 0;
    minsolrep = subnc;
    Return[Null]
  ];

  minimumResult = Quiet@FindMinimum[
    normedrel . normedrel,
  ...
  If[! MatchQ[minimumResult, {_?NumericQ, {___Rule}}],
    Message[FindSolUptdAsymptW::failed];
    Throw[$Failed]
  ];
  {minim, minsolrep} = minimumResult;
];
...
MyFindRoot := Block[{},
  ...
  (* This refinement only returns MYminsolrep; no evaluation history is used. *)
];


makeStr[list_]:=StringJoin[ToString/@list];
exportToJuliaInitialData[SYT_, includeJacobian_: True]:=Block[{},
...
(* Julia consumes the exported Jacobian; Bertini differentiates its own system. *)
If[TrueQ[includeJacobian],
...
mytable =  {"syt","lambda0","var", "Initialvar", "expression"};
If[TrueQ[includeJacobian],
  table = MapThread[Append, {table, cleanJacobian}];
  AppendTo[mytable, "jacobian"];
];
...
(* Parse decimal digits exactly before rounding; CSV numeric import otherwise
   converts long Bertini decimal strings to machine-precision numbers. *)
ClearAll[ParseContinuationReal];
ParseContinuationReal[text_String, digits_] := Module[
  {normalized, parts, mantissa, exponent, sign, fractionDigits, integer},
  normalized = StringReplace[StringTrim[text], {"*^" -> "e", "E" -> "e"}];
  If[! StringMatchQ[normalized,
    RegularExpression["[+-]?(?:[0-9]+(?:\\.[0-9]*)?|\\.[0-9]+)(?:e[+-]?[0-9]+)?"]],
    Return[$Failed]
  ];
  parts = StringSplit[normalized, "e"];
  mantissa = First[parts];
  exponent = If[Length[parts] == 2, ToExpression[Last[parts]], 0];
  sign = If[StringStartsQ[mantissa, "-"], -1, 1];
  mantissa = StringTrim[mantissa, "+" | "-"];
  fractionDigits = If[StringContainsQ[mantissa, "."],
    StringLength[mantissa] - First[First[StringPosition[mantissa, "."]]], 0];
  integer = FromDigits[StringReplace[mantissa, "." -> ""]];
  N[sign integer 10^(exponent - fractionDigits), digits]
];
ParseContinuationReal[_, _] := $Failed;
...
ReadJuliaResultVector[] := Module[
  ...
  imported = Quiet@Check[Import[resultFile, "CSV", "Numeric" -> False], $Failed];
  If[imported === $Failed || ! ListQ[imported] || Length[imported] < 2,
    Return[$Failed]
  ];

  If[! AllTrue[Rest[imported], ListQ[#] && Length[#] >= 2 &], Return[$Failed]];
  values = imported[[2 ;;, 2]];
  If[! ListQ[values] || values === {},
    Return[$Failed]
  ];

  values = ParseContinuationReal[#, prec] & /@ values;
...
Ccoeff:=Block[{},ccoeffattarget=c2susycoeff/.sol[0]//Flatten;
relstosolve=Allrel/.sym[n_]:>0/.ccoeffattarget;
solvedvars=ccoeffattarget;
While[
Length@(currentrels=Select[relstosolve//Chop,Length[Variables[#]]==1&])>0,
currentvars=Variables/@currentrels//Flatten;
currentrels2=Table[SelectFirst[currentrels,(Variables[#][[1]])==currentvars[[i]]&],{i,Length[currentvars]}];
solvedcurrentvars=Solve[currentrels2==0,currentvars][[1]];
solvedvars=Join[solvedvars,solvedcurrentvars];
relstosolve=relstosolve/.solvedcurrentvars;];solvedvars]
...
SolutionFromSYTUptdStart[SYT_] := Block[{
  ...
  (* GenerateSUSYWsystem initializes the Q-system itself. *)
  generateQsystemSeconds = 0;
