...
RUN_SINGLE="$ROOT_DIR/wolfram/RunSingle.wl"
...
RESULT_MERGE_EVERY="${RESULT_MERGE_EVERY:-100}"
RESULT_MERGE_INTERVAL="${RESULT_MERGE_INTERVAL:-60}"
RESULT_SYT_DIR="${BERTINI_RESULT_SYT_DIR:-$RESULTS_DIR}"
...
usage() {
  cat <<'USAGE'
Usage:
  bash scripts/submit_run_single_jobs.sh --yd '{2,2,1}' -j 8
  bash scripts/submit_run_single_jobs.sh --yd '{5,4,3,2,1}_1' -j 100
  bash scripts/submit_run_single_jobs.sh --yd '{5,4,3,2,1}_1' -n 9 -j 100
  bash scripts/submit_run_single_jobs.sh --list data/tableaux/{2,2,1}.txt -j 8

Options:
  --yd YD              Young diagram in Mathematica InputForm, e.g. '{2,2,1}'.
                       Add _N suffix to run partition N of 9, e.g. '{5,4,3,2,1}_4'.
                       Creates/updates data/tableaux/{2,2,1}.txt if --list is absent.
  ...
  --logs DIR           Directory for stdout/stderr logs. Default: workspace/logs/RunSingle,
                       or workspace/logs/RunSingle/<YD>_part_<N>_of_<M> for partitioned runs.
  --no-logs            Do not write per-SYT stdout/stderr logs under workspace/logs/RunSingle.
  ...
  --cleanup-runs       Delete each successful workspace/runs/syt-* work directory after
...
Environment:
  ...
  BERTINI_RESULT_SYT_DIR       Default: <project directory>/workspace/results.
  SYT_LOCK_STALE_SECONDS       Age for reclaiming remote locks. Default: 21600.
  RESULT_MERGE_EVERY          Merge after this many completion waits. Default: 100.
  RESULT_MERGE_INTERVAL       Merge after this many seconds at a completion. Default: 60.
                             A final merge always runs; snapshots remain resumable.
USAGE
}
...
WL
  ...
  if "$WOLFRAM_KERNEL" -noprompt -script "$generator" "$yd" "$output_file" "$TABLEAUX_DIR"; then
    status=0
  else
    status=$?
  fi
...
merge_completed_results() {
  "$PYTHON_EXECUTABLE" -m bae_bertini.results \
    "$JOB_RESULT_DIR" "$YD" "$AGGREGATE_RESULT_FILE" "$FAIL_RESULT_FILE"
}

maybe_merge_completed_results() {
  if ((completed_since_merge >= RESULT_MERGE_EVERY || SECONDS - last_merge_seconds >= RESULT_MERGE_INTERVAL)); then
    if ! merge_completed_results; then
      failures=$((failures + 1))
    fi
    completed_since_merge=0
    last_merge_seconds=$SECONDS
  fi
}
...
[[ "$RESULT_MERGE_EVERY" =~ ^[1-9][0-9]*$ ]] || die "RESULT_MERGE_EVERY must be a positive integer"
[[ "$RESULT_MERGE_INTERVAL" =~ ^[1-9][0-9]*$ ]] || die "RESULT_MERGE_INTERVAL must be a positive integer"
[[ -f "$ROOT_DIR/src/bae_bertini/results.py" ]] || die "missing src/bae_bertini/results.py"
...
if [[ -z "$LIST_FILE" ]]; then
  ...
  LIST_FILE="$TABLEAUX_DIR/${yd_key}.txt"
...
JOB_RESULT_DIR="$RUNS_DIR/batch-results/$yd_key"

if [[ -n "$PART_INDEX" && "$LOG_DIR_SET" -eq 0 ]]; then
  LOG_DIR="$LOGS_DIR/RunSingle/${yd_key}_part_${PART_INDEX}_of_${PART_COUNT}"
fi
...
while IFS= read -r syt || [[ -n "$syt" ]]; do
  ...
  while (( $(jobs -pr | wc -l) >= MAX_JOBS )); do
    ...
    completed_since_merge=$((completed_since_merge + 1))
  done
  ...
  # Refill the worker slot before periodically consolidating completed results.
  maybe_merge_completed_results
done < "$LIST_FILE"
