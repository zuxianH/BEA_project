...
HELPER="$ROOT_DIR/wolfram/crosscheck.wl"
COMPARE_HELPER="$ROOT_DIR/src/bae_bertini/crosscheck.py"
TOLERANCE="1e-5"
KEEP_WORKDIR=0
SYT=""
SAVED_RESULT_DIR="${CROSSCHECK_RESULT_SYT_DIR:-$RESULTS_DIR}"
...
Options:
  ...
  --keep-workdir      Keep temporary CSVs and logs under workspace/runs/crosscheck-*.
...
The flipped result is saved in workspace/results. Override its destination with
...
[[ -f "$RUN_SINGLE" && -f "$HELPER" && -f "$COMPARE_HELPER" ]] || \
  die "missing RunSingle.wl, crosscheck.wl, or src/bae_bertini/crosscheck.py"
...
mkdir -p "$RUNS_DIR"
WORK_DIR="$(mktemp -d "$RUNS_DIR/crosscheck-XXXXXX")"
...
set +e
"$PYTHON" -m bae_bertini.crosscheck "$RESULT_DIR" "$SYT" "$FLIP_SYT" "$TOLERANCE" \
  --report "$REPORT_FILE" \
