...
DEFAULT_INPUT = EXAMPLES / "{2, 0}_{{1, 3, 4, 5}, {2, 6}}.csv"
...
def bertini_complex_real_text(value, tolerance="1e-8"):
    """Export real coefficients without silently projecting a complex endpoint."""
    text = bertini_complex_text(value).strip()
    match = re.fullmatch(r"\(([^,]+),\s*([^,]+)\)", text)
    if not match:
        raise ValueError(f"Unexpected Bertini complex representation: {text!r}")
    real_text, imaginary_text = (part.strip() for part in match.groups())
    real, imaginary = Decimal(real_text), Decimal(imaginary_text)
    allowed = Decimal(str(tolerance))
    if not allowed.is_finite() or allowed < 0:
        raise ValueError("Real-coefficient tolerance must be finite and nonnegative")
    if not real.is_finite() or not imaginary.is_finite():
        raise ValueError(f"Non-finite Bertini coefficient: {text}")
    if abs(imaginary) > allowed * max(Decimal(1), abs(real)):
        raise ValueError(f"Bertini coefficient is not real within tolerance {tolerance}: {text}")
    return real_text


@contextmanager
def atomic_csv_file(output_path):
    """Publish a complete CSV with a rename on the same filesystem."""
...
def parse_expression(expression, namespace):
    ...
    # AST column offsets are UTF-8 bytes. Encode each physical line once rather
    # than scanning and encoding the full expression for every numeric literal.
    source_lines = expression.encode('utf-8').replace(b'\r\n', b'\n').replace(b'\r', b'\n').split(b'\n')
...
def write_result(
    ...
    with atomic_csv_file(output_path) as csv_file:
        fieldnames = [
            "source_file",
            "syt",
            "var",
...
def write_legacy_result(output_path, variable_names, endpoint, tolerance="1e-8"):
    if len(variable_names) != len(endpoint):
        raise ValueError("Endpoint length does not match the variable count")
    with atomic_csv_file(output_path) as csv_file:
        ...
        for name, value in zip(variable_names, endpoint):
            writer.writerow(
                {
                    "var": name,
                    "b_final_value": bertini_complex_real_text(value, tolerance),
                }
            )
...
def run_one(input_path, args):
    ...
    if args.legacy_output:
        write_legacy_result(output_path, variable_names, endpoint, args.tracking_tolerance)
    else:
        write_result(
