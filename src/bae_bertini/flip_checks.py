...
def hausdorff_distance(
    ...
    # A square root is monotone: defer it until after both min/max reductions.
    # Each pair contributes to both directed distances, so evaluate it once.
    left_min = [Decimal("Infinity")] * len(left)
    right_min = [Decimal("Infinity")] * len(right)
    for i, (left_real, left_imaginary) in enumerate(left):
        for j, (right_real, right_imaginary) in enumerate(right):
            real_delta = left_real - right_real
            imaginary_delta = left_imaginary - right_imaginary
            squared = real_delta * real_delta + imaginary_delta * imaginary_delta
            left_min[i] = min(left_min[i], squared)
            right_min[j] = min(right_min[j], squared)
    return max(max(left_min), max(right_min)).sqrt()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "csv_or_yd",
        help="Aggregate CSV path, or Young diagram like '{10,6}' for data/references/all_SYT_{10,6}.csv.",
    )
    parser.add_argument("--result-dir", type=Path, default=REFERENCES)
